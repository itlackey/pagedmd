...
export default function plugin1(md) {}
export const metadata = { name: 'plugin1', version: '1.0.0' };
export const css = '.plugin1 { color: red; }';
`;

      const plugin2 = `
export default function plugin2(md) {}
export const metadata = { name: 'plugin2', version: '1.0.0' };
export const css = '.plugin2 { color: blue; }';
`;
...
`
      ...
      const config: ResolvedConfig = {
        input: TEST_DIR,
        plugins: [
          { path: './plugins/plugin1.js', type: 'local', enabled: true },
          { path: './plugins/plugin2.js', type: 'local', enabled: true },
        ],
        title: 'Test',
        authors: ['Test Author'],
        timeout: 60000,
      ...
      expect(html).toContain('.plugin1 { color: red; }');
      expect(html).toContain('.plugin2 { color: blue; }');
      expect(html).toContain('/* Plugin: plugin1 v1.0.0 */');
      expect(html).toContain('/* Plugin: plugin2 v1.0.0 */');
    });
...
export const css = '.plugin-style { color: red; }';
`;

      await mkdir(path.join(TEST_DIR, 'plugins'), { recursive: true });
      await writeFile(path.join(TEST_DIR, 'plugins/test.js'), plugin);
...
`
      ...
      const config: ResolvedConfig = {
        input: TEST_DIR,
        plugins: [{ path: './plugins/test.js', type: 'local', enabled: true }],
        styles: ['custom.css'],
        title: 'Test',
        authors: ['Test Author'],
        timeout: 60000,
      ...
      const pluginCssIndex = html.indexOf('.plugin-style');
