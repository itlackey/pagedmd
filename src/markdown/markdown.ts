...
import { ResolvedConfig } from '../config/config-state.ts';
...
export async function processMarkdownFiles(
    ...
    // Configure markdown engine based on manifest extensions
    const extensionOptions = parseExtensionOptions(config.extensions);
    const md = createPagedMarkdownEngine(extensionOptions);
